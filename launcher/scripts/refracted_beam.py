...
class Refracted(object):
    ...
    def fit_refracted_pixel(self):
        ...
        for i in range(self.p_vs_t.shape[1]):
            ...
            a = fit.params["g_amplitude"].value  # noqa: F841
    ...
    def fit_offset_and_pixel(self, margin=0.5):
        ...
        fit = lmfit.minimize(
            ...
            args=(wl[:index_c],),
            method=engine,
            kws={"data": self.refracted_pixel[:index_c], "err": self.d_refracted_pixel[:index_c]},
        )
    ...
    def plot_refraction_map(self, output_dir=None):
        ...
        im = ax.pcolormesh(self.wl_boundaries, _theta_boundaries, np.log(self.p_vs_t), cmap=cmap)  # noqa: F841
    ...
    def improve_peaks(self, output_dir=None):
        ...
        a = fit.params["g_amplitude"]  # noqa: F841
        ...
        a = fit_r.params["g_amplitude"]  # noqa: F841
    ...
    def plot_pixel_vs_tof(self, output_dir=None):
        ...
        im = ax.pcolormesh(self.tof_boundaries, np.arange(self.p_vs_t.shape[0] + 1), np.log(self.p_vs_t), cmap=cmap)  # noqa: F841
        ax.set_title("pixel vs TOF")

        ax.scatter(
            self.tof_boundaries, np.ones(self.tof_boundaries.shape) * self.center_r, zorder=1, linewidth=2, s=0.2
        )
        ax.scatter(
            self.tof_boundaries, np.ones(self.tof_boundaries.shape) * self.center_db, zorder=1, linewidth=2, s=0.2
        )
    ...
    def compute_reflectivity(self, output_dir):
        ...
        np.savetxt(os.path.join(output_dir, "%s-reflectivity.txt" % self.run_number), np.asarray([q, ref, err]).T)
...
def process_run(run_number, output_dir, material="Si"):
    ...
    fitted_pixel_prediction = refr.pixel_prediction_with_size(refr.wl_center, offset=_offset, pixel_size=_pixel_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--material", type=str, dest="material", help="Si or Quartz", required=False, default="Si")
