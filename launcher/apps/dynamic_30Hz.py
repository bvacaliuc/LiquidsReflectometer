...
class Dynamic30Hz(QWidget):
    ...
    def load_settings_from_file(self):
        ...
        """
        _settings_file, _ = QFileDialog.getOpenFileName(
            self, "Open file", self.output_dir_label.text(), "Settings file (*.json)"
        )
    ...
    def ref_selection(self):
        _ref_file, _ = QFileDialog.getOpenFileName(
            self, "Open file", self.ref_path.text(), "60Hz reference file (*.txt)"
        )
