...
def example_template_reduction():
    ...
    results = new_template.reduce_from_template(
        211029,
        template_path / "test_template.xml",
        "IPTS-36119",
        datapath=datapath,
...
def example_template_reduction_3ang():
    ...
    for idx, run in enumerate(runs):
        ...
        results = new_template.reduce_from_template(
            run,
            template_path / "test_template_3ang.xml",
            "IPTS-36119",
            datapath=datapath,
            template_path=template_path,
            override_params=override_params,
...
def example_template_reduction_new():
    ...
    for idx, run in enumerate(runs):
        #    # This needs to include anything you want to set that isn't in the template file or you want to deviate from the template file.
        override_params = {"Spath": Spath, "NEXUSpathRB": datapath, "DBpath": DBpath}

        results = new_template.reduce_from_template(
            run,
            template_path / "REFL_211029_template_new.xml",
            "IPTS-36119",
            datapath=datapath,
            template_path=template_path,
            override_params=override_params,
