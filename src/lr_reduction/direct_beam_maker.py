...
class Direct_Beam:
    def __init__(self, experiment_id=None, NEXUSpath=None, savepath=None):
        ...
        self.Cd = [57.5, 126.5, 126.5 + 123.0, 2 * 126.5 + 2 * 123.0]  # microns Cd for each attenuator
        ...
        self.t0 = [
            0.114,
            0.0295,
        ]  # for a linear fit to emission time as a function of lambda: t0[0]+t0[1]*Lambda=emission time
    ...
    def create_db(
        self, run_list, save_name, plot=True, mu_file=None, flip_atten=False, return_traces=False, experiment_id=None
    ):
        """
        ...
        if not nexus_base or not save_base:
            raise ValueError(
                "NEXUSpath/savepath not set: provide experiment_id or supply NEXUSpath/savepath when constructing Direct_Beam or call create_db with experiment_id"
            )
        ...
        for i, run in enumerate(run_list):
            ...
            tof_array, y_tof_corr, error_array_corr, log_values, DTC_corr = BP.convert_to_binary(
                fname, self.low_res, collapse_x=True, tofbin=50, tofmax=100000, tofmin=0, deadtime=4.2, tof_step=100
            )
            T = tof_array * 1000
            DTC = DTC_corr
    ...
    def _extract_cd_values(self, log_values, flip_atten=False):
        ...
        Cd_thickness = (
            self.Cd[0] * Atten[0] + self.Cd[1] * Atten[1] + self.Cd[2] * Atten[2] + self.Cd[3] * Atten[3]
        ) * 1e-4  # cm
