...
def convert_to_binary(
    fname, lowres, collapse_x=True, tofbin=50, tofmax=100000, tofmin=0, deadtime=4.2, tof_step=100, n_y=304, n_x=256
):
    """
    ...
    tof_array, DTC, error_counts = get_deadtime_correction(
        error_event_offset, e_offset, cPc, tofbin, tofmax, tofmin, deadtime=deadtime, tof_step=tof_step
    )
...
def get_log_values(fname):
    ...
    log_values["op_mode"] = f["entry/DASlogs/BL4B:CS:ExpPl:OperatingMode/value"][
        0
    ]  # This is one that can say "Free Liquid"
    try:
        log_values["coordinates"] = f["entry/DASlogs/BL4B:CS:Mode:Coordinates/value"][
            0
        ]  # This is one that shows earth vs beam center 0=earth; 1=beam
    except:
