...
def get_default_template_file(output_dir: str, tthd: float) -> str:
    """
    Determine the default template file based on the tthd value.

    @param output_dir: Directory with template files
    ...
    if tthd > 0:
        template_path = output_dir / "template_up.xml"
    else:
        template_path = output_dir / "template_down.xml"

    default_template_path = output_dir / "template.xml"

    if template_path.exists():
        template_file = str(template_path)
    elif default_template_path.exists():
        template_file = str(default_template_path)
